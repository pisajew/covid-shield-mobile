...
const Content = ({ title, body, notCoveredList, coveredList, externalLinkText, externalLinkCTA }: ContentProps) => {
  ...
  return (
    <Box>
      ...
      {notCoveredList && notCoveredList.map(item => <BulletPoint key={item.text} listAccessibile={item.listAccessibile} text={item.text} />)}
      {coveredList && coveredList.map(item => <NumberListItem key={item.text} number={item.number} listAccessibile={item.listAccessibile} text={item.text} />)}
      {externalLinkButton}
    </Box>
  );
};
    ...
    case 'regionCovered':
      content = (
        <Content
          ...
          coveredList={[
            { number: '1.', listAccessibile: 'listStart', text: i18n.translate(`DataUpload.NoCode.RegionCovered.${region}.Body2`) },
            { number: '2.', listAccessibile: 'item', text: i18n.translate(`DataUpload.NoCode.RegionCovered.${region}.Body3`) },
            { number: '3.', listAccessibile: 'item', text: i18n.translate(`DataUpload.NoCode.RegionCovered.${region}.Body4`) },
            { number: '4.', listAccessibile: 'listEnd', text: i18n.translate(`DataUpload.NoCode.RegionCovered.${region}.Body5`) },
          ]}
